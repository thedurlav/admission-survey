...
export default function AdminDashboard() {
  ...
  const [newSurveyor, setNewSurveyor] = useState({ username: '', password: '', fullName: '' });
  ...
  const validateSurveyor = () => {
    ...
    else if (!/^[a-z0-9_]+$/.test(newSurveyor.username)) e.username = 'Only lowercase letters, numbers, underscores';
    if (!newSurveyor.password) e.password = 'Password required';
    else if (newSurveyor.password.length < 6) e.password = 'At least 6 characters';
    return e;
  };
  ...
  const handleAddSurveyor = async (e) => {
    ...
    if (Object.keys(errs).length) { setSurveyorErrors(errs); return; }
  ...
  const filteredStudents = students.filter(s =>
    ...
    (s.surveyedBy?.fullName || '').toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="page">
      ...
      <div className="nav-tabs">
        <button className={`nav-tab ${tab === 'students' ? 'active' : ''}`} onClick={() => setTab('students')}>
          📊 All Student Data
        </button>
        <button className={`nav-tab ${tab === 'surveyors' ? 'active' : ''}`} onClick={() => setTab('surveyors')}>
          👥 Surveyors
        </button>
        ...
        {tab === 'students' && (
          <div className="fade-in">
            ...
            <div className="card">
              ...
              {loadingStudents ? (
                <div className="loading"><div className="spinner"></div> Loading data...</div>
              ) : (
                <StudentTable
        ...
        {tab === 'surveyors' && (
          <div className="fade-in">
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1rem' }}>
              <button className="btn btn-primary" onClick={() => setShowAddSurveyor(true)}>
                ＋ Add Surveyor
              </button>
            </div>

            {loadingSurveyors ? (
              <div className="loading"><div className="spinner"></div> Loading surveyors...</div>
            ) : surveyors.length === 0 ? (
              <div className="empty">
                <div className="empty-icon">👥</div>
                <div className="empty-text">No surveyors found. Add one to get started.</div>
              </div>
            ) : (
                  ...
                  <div key={s._id} className="surveyor-card fade-in">
                    ...
                    <div className="surveyor-info">
                      ...
                      <div className="surveyor-actions">
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => handleDeleteSurveyor(s._id)}>
                          Delete
                        </button>
                      </div>
                    </div>
        ...
        {tab === 'add' && (
          <div className="card fade-in">
            ...
            <div className="card-body">
              <StudentForm onSuccess={() => { loadStudents(); setTab('students'); }} />
            </div>
          </div>
        )}
      </div>
      ...
      {/* ── ADD SURVEYOR MODAL ── */}
      {showAddSurveyor && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowAddSurveyor(false)}>
          <div className="modal fade-in">
            <div className="modal-header">
              <div className="modal-title">Add New Surveyor</div>
              <button className="modal-close" onClick={() => setShowAddSurveyor(false)}>×</button>
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={e => { setNewSurveyor(s => ({ ...s, fullName: e.target.value })); setSurveyorErrors(er => ({ ...er, fullName: '' })); }}
                      placeholder="e.g. Ravi Kumar"
                      className={surveyorErrors.fullName ? 'input-error' : ''}
                    />
                    {surveyorErrors.fullName && <span className="field-error">{surveyorErrors.fullName}</span>}
                  </div>
                  <div className="form-group">
                    <label>Username *</label>
                    <input
                      value={newSurveyor.username}
                      onChange={e => { setNewSurveyor(s => ({ ...s, username: e.target.value.toLowerCase() })); setSurveyorErrors(er => ({ ...er, username: '' })); }}
                      placeholder="e.g. ravi_kumar (lowercase only)"
                      className={surveyorErrors.username ? 'input-error' : ''}
                    />
                    {surveyorErrors.username && <span className="field-error">{surveyorErrors.username}</span>}
                  </div>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={e => { setNewSurveyor(s => ({ ...s, password: e.target.value })); setSurveyorErrors(er => ({ ...er, password: '' })); }}
                      placeholder="Min 6 characters"
                      className={surveyorErrors.password ? 'input-error' : ''}
                    />
                    {surveyorErrors.password && <span className="field-error">{surveyorErrors.password}</span>}
                  </div>
                </div>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setShowAddSurveyor(false)}>Cancel</button>
                <button type="submit" className="btn btn-primary" disabled={savingSurveyor}>
